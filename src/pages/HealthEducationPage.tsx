    ...
    <div className="space-y-6 text-left animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs hover-lift">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
              {language === 'ta' ? 'சுகாதார விழிப்புணர்வு & கல்வி' : 'Health Education & Guides'}
            </h1>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          ...
          <button
            ...
            className="px-4 py-2 rounded-full text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-xs transition-all cursor-pointer btn-lift"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredArticles.map((art, idx) => {
          ...
          return (
            <div
              key={art.id}
              className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs hover-lift transition-all flex flex-col justify-between group animate-fade-in-up hover:border-blue-300"
            >
              ...
              <div className="pt-4 mt-4 border-t border-slate-100 flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-5 py-2.5 rounded-full text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white flex items-center gap-1.5 shadow-xs transition-colors cursor-pointer btn-lift"
                >
      ...
      {selectedArticle && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-scale-up">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl max-h-[90vh] overflow-y-auto space-y-4 text-left">
            <div className="flex items-start justify-between gap-4 pb-3 border-b border-slate-100">
              ...
              <button
                ...
                className="p-2 rounded-full text-slate-400 hover:bg-slate-100 shrink-0 cursor-pointer transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-2 flex-wrap">
              <button
                ...
                className="px-5 py-2.5 rounded-full text-xs font-bold border border-blue-600 text-blue-900 hover:bg-blue-50 flex items-center gap-1.5 cursor-pointer btn-lift transition-colors"
              >
              ...
              <button
                type="button"
                onClick={() => setSelectedArticle(null)}
                className="px-6 py-2.5 rounded-full text-xs font-bold bg-slate-100 hover:bg-slate-200 text-slate-700 cursor-pointer transition-colors"
              >
                Close
              </button>
            </div>
